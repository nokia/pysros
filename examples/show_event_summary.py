...
Obtain a summary of the event history on the node.

Execution on SR OS
    usage: pyexec show_event_summary.py
Execution on remote machine
    usage: python show_event_summary username@host
Execution on remote machine if show_port_counters.py is executable
     usage: ./show_event_summary.py username@host

Add the following alias so that the Python application can be run as a
native MD-CLI command.

/configure python { python-script "show-event-summary" admin-state enable }
/configure python { python-script "show-event-summary" urls ["cf3:show_event_summary.py"]
/configure python { python-script "show-event-summary" version python3 }
...
/configure system { management-interface cli md-cli environment command-alias alias "event-summary" description "Show event summary" }
/configure system { management-interface cli md-cli environment command-alias alias "event-summary" python-script "show-event-summary" }
/configure system { management-interface cli md-cli environment command-alias alias "event-summary" mount-point "/show" }
...
def getter(connection, path):
    """Obtain modelled data from a device."""
    import sys  # pylint: disable=import-outside-toplevel

...
def get_input_args():
    ...
    if len(sys.argv) > 1:
        ...
        # the application.
            ...
            print("Argument is not valid IP address: ", e)
            sys.exit(-1)
...
def get_chassis_inventory(c):

    # Obtain the data from the SR OS device.  The data in this example is only obtained
    # from the first chassis per host.
    chassis_state = c.running.get('/nokia-state:state/chassis[chassis-class="router"][chassis-number="1"]')
    ...
    hash_data = {type: {number: build_hash}}
    ...
    for item in chassis_state.keys():
        if item in ["fan", "power-supply"]:
            ...
            for item_num in chassis_state[item]:
                item_num_hash[item_num] = get_hardware_data(chassis_state[item][item_num])
...
# required data in order to build the inventory.
def get_card_inventory(c):

    # Obtain the state data about line cards from the SR OS device.
    card_state = c.running.get('/nokia-state:state/card')
...
def main():
    ...
    # Check that the application is not being executed on SR OS.  If it is, provide a warning and exit.
    if sros() == True:
        print("This application cannot be run on the SR OS node.  Please run it on an external device")
        sys.exit(-2)
    else:
        ...
        for host in hosts:
            ...
            c = get_connection(str(host))
            # Obtain the inventory of chassis, fan and power-supply data.
            per_host_inv = get_chassis_inventory(c)
            # Obtain the inventory of line card data.
            per_host_inv.update(get_card_inventory(c))

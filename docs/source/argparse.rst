:mod:`argparse` -- Argument parser functions
============================================

.. module:: argparse
   :synopsis: Argument parser functions adapted for SR OS

.. admonition:: Differences to CPython
   :class: attention

   This module implements a subset of the corresponding CPython 3.12 module,
   as described below.  For more information, refer to the original CPython
   documentation: `argparse (version 3.12) <https://docs.python.org/3.12/library/argparse.html>`_.
   Class FileType is not implemented due to differences between CPython and MicroPython.

This module is used when executing on SR OS only.  On a remote machine, the
native Python `time <https://docs.python.org/3/library/argparse.html>`_ module is
used.

The :mod:`argparse` module provides functions for providing rich user input enhancements to
the command-line interface of any application.  It allows for the capture and validation of
arguments along with the provision of informative help text.  The module issues 
errors when users give the program invalid arguments.

.. note:: 

   This documentation is adapted from the original CPython documentation: 
   `argparse (version 3.12) <https://docs.python.org/3.12/library/argparse.html>`_ removing
   and updating unsupported or adjusted functions to support the MicroPython interpreter 
   available on SR OS.
...
choices_                     Limit values to a specific set of choices                   ``['foo', 'bar']``, ``range(1, 10)``, or ``Container`` instance
...
nargs_                       Number of times the argument can be used                    ``int``, ``'?'``, ``'*'``, or ``'+'``
required_                    Indicate whether an argument is required or optional        ``True`` or ``False``
:ref:`type <argparse-type>`  Automatically convert an argument to the given type         ``int``, ``float``, or callable function
============================ =========================================================== ==========================================================================================================================
...
.. warning::
   When using file paths, please always specify files as URLs for SR OS, e.g., cf3:\\file_to_path.
...
arguments they contain.  For example::
   ...
   >>> with open(r'cf3:\args.txt', 'w') as fp:
   ...     fp.write('-f\nbar')
   ...
   >>> parser = argparse.ArgumentParser(fromfile_prefix_chars='@')
   >>> parser.add_argument('-f')
   >>> parser.parse_args(['-f', 'foo', r'@cf3:\args.txt'])
   Namespace(f='bar')

Arguments read from a file must by default be one per line (but see also
:meth:`~ArgumentParser.convert_arg_line_to_args`) and are treated as if they
were in the same place as the original file referencing argument on the command
line.  So in the example above, the expression ``['-f', 'foo', r'@cf3:\args.txt']``
...
.. warning::
   ...
   Bound methods of built-in types (e.g., str.lower, int.from_bytes) cannot be used in the type argument due to MicroPython implementation limitations.
...
attributes, you can use the standard Python idiom, ``__dict__``::
